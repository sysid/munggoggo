...
class Base:
    __slots__ = 'loop', 'parent', '__future_store', 'closing'

    def __init__(self, *, parent: 'Base' = None):
        self.parent = parent
        self.loop = None
        self.__future_store = None
        self.closing = None

    async def init_asyncio(self):
        """ late setup necessary to get final event-loop (e.g. set be uvcorn) """
        self.loop = asyncio.get_event_loop()

        if self.parent:
            self.__future_store = self.parent._future_store_child()
        else:
